...
def chat(uuid: str, body: str):
    ...
    try:
        messages = mdb.cursor().execute('SELECT creator, content FROM messages LIMIT 20 SORT BY aid DESC').fetchall()

    finally:
        mdb.cursor().execute('CREATE TABLE IF NOT EXISTS  messages {aid BIGINT NOT NULL autoincrement, creator char(5),content text, PRIMARY KEY aid} charset=utf8;')
