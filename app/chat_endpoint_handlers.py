...
async def chat_image_internal(chat_id: str, user_id: str):
    ...
    sql_connection.cursor().execute(f"ALTER TABLE `{mariadb_name(user_id, chat_id)}`.`images` CHANGE COLUMN content content LONGTEXT;")
    if ENABLE_MESSAGE_LIMITS:
        cursor = sql_connection.cursor()
        cursor.execute(
            "SELECT remaining_messages, additional_remaining_messages FROM chat_users.users WHERE user_id=?;",
    ...
    sql_connection.ping()
    sql_connection.cursor().execute(f"INSERT INTO `{mariadb_name(user_id, chat_id)}`.images (id, content, alt) VALUES (?, ?, ?);", (image_id, response["image"], response["content"]))
